...
--[[			if obj == nil then 
			...
			local info = {}
			if screen[x][y] ~= nil then info = screen[x][y] end
			...
			if obj == nil then 
				...
				if green == 1 then info.g = curMap.bg end
			else
				if curMap == map then
					info.r = obj.r
				elseif curMap == sMap then
				...
				if green == 1 then info.g = obj.g end
			end
			if obj ~= nil and obj.anim ~= nil then
				info.an = obj.anim
			end
			if green == 1 and info.a ~= nil then
				info.a = info.a + ((dist*3) / (i + 1)) / dist
			else
				info.a = ((dist*3) / (i + 1)) / dist
			end
